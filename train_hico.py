...
def make_train_dataset_old(args, tokenizer, accelerator):
...
def main(args):
    ...
    base_encoder_hidden_states = text_encoder(clip_inputs['input_ids'].to(accelerator.device))[0]   # [1, 77, 768]
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                for cid in range(bsize):
                    ...
                    # bsid = min(batch_cond["num_selected"][cid], 4)
                    dot_noisy_latents = torch.repeat_interleave(torch.unsqueeze(noisy_latents[cid], dim=0), repeats=bsid, dim=0)
                    dot_timesteps = torch.repeat_interleave(torch.unsqueeze(timesteps[cid], dim=0), repeats=bsid, dim=0)
                    ...
                    ########## fuse - avg ##########
                    dot_down_block_res_sample = [torch.sum(down_block_res_samples[iii], dim=0)/ bsid for iii in range(len(down_block_res_samples))]  
                ...
                for iix in range(len(bs_down_block_res_sample[0])):
                    tmp_down = [bs_down_block_res_sample[iiy][iix] for iiy in range(len(bs_down_block_res_sample))]
                ...
                # unet empty
                unet_encoder_hidden_states = torch.repeat_interleave(base_encoder_hidden_states, repeats = latents.shape[0], dim=0)
                ...
                model_pred = unet(
                    ...
                    down_block_additional_residuals=[
                        sample.to(dtype=weight_dtype) for sample in fus_down_block_res_samples
                    ],
                    mid_block_additional_residual=fus_mid_block_res_sample.to(dtype=weight_dtype),
                ).sample
                #del fus_down_block_res_samples
                #del fus_mid_block_res_sample
